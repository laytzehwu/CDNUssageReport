describe("Menu Spec", function () {
    var Menu = CDNUsageReport.Views.Menu;
    var mockObj = null;
    beforeEach(function () {
        mockObj = {
            clickEvent: function () {}
...
describe("UsageView Spec", function () {
   ...
   it("Render full columns header", function () {
	...
	// Only show 1 column
	var $row = CDNUsageReport.Views.UssageView.renderGridHeader({columns: {publisher: true}});
	expect($row.hasClass("grid-header")).toBeTruthy();
	var $cells = $row.find(".grid-cell");
	var labels = CDNUsageReport.Views.UssageView.getLabels();
	var labelKeys = Object.keys(labels);
	...
	// Show 2 columns
	var $row = CDNUsageReport.Views.UssageView.renderGridHeader({columns: {publisher: true, bytesCached: true, requestCached: true}});
	expect($row.hasClass("grid-header")).toBeTruthy();
	var $cells = $row.find(".grid-cell");
	var labels = CDNUsageReport.Views.UssageView.getLabels();
	var labelKeys = Object.keys(labels);
    ...
    it("Render ussage row by single use case properly", function () {
    	...
    	var $row = CDNUsageReport.Views.UssageView.renderRow(useCase);
    ...
    it("Render ussage full detail", function () {
    	var useCase = new UseCaseModel([1,1,1,50,50,50,50]);
    	var $table = CDNUsageReport.Views.UssageView.renderDetailDialog(useCase);
    ...
    it("Render ussage list with full columns", function () {
    	...
    	var $list = CDNUsageReport.Views.UssageView.renderList(cases);
